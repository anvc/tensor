				      ...
				      callback: function() {
				    	var $content = $('.add_metadata_bootbox #bootbox-content:first');
		    	  		...
		    	  		$.each(selected, function() {
                            ...
                            var $insert;
                            // Custom to Tensor
                            $insert = $('<div class="form-group"><label for="'+val+'" class="col-sm-3 control-label">'+val+'</label><div class="col-sm-9"><input class="form-control" id="'+val+'" value="" type="text"></div></div>');
                            // End custom to Tensor
		    	  			$insert_into.append($insert);
		    	  		});
		    	  		$('.add_metadata_bootbox').modal('hide').data('bs.modal', null).on('hidden.bs.modal', function() {
